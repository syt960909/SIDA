...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="input path_training parameters_save path")
    parser.add_argument('--data_path',default="./myData_pancreatic_5batches.txt", help = "your path of myData_pancreatic_5batches.txt")
    parser.add_argument('--metadata_path', default="./mySample_pancreatic_5batches.txt", help="your path of mySample_pancreatic_5batches.txt")
    parser.add_argument('--epoch', type=int,default=15)
    parser.add_argument('--batch_size', type=int,default=512)
    parser.add_argument('--model_save_path', default="./your_model.h5")
    parser.add_argument('--siamese_g_save_path', default="./your_siamese_model_g.h5")
...
if __name__ == '__main__':
    args = parse_args()
    adata = pd.read_csv(args.data_path,sep='\t',header=0, index_col=0)
    adata = sc.AnnData(np.transpose(adata))
    d4_genes = list(adata.var_names)
    sample_adata = pd.read_csv(args.metadata_path,header=0, index_col=0, sep='\t')
    ...
    model1.save(args.siamese_g_save_path)